...
public class ApprovalService {
	...
	public Approval getDraftInfoOne(ApprovalDto dto) {
		...
		Approval approval = Approval.builder().apprNo(dto.getAppr_no()).apprTitle(dto.getAppr_title())
				.apprContent(dto.getAppr_content()).apprWriterName(emp.getEmpName())
				.apprHoliStart(dto.getAppr_holi_start()).apprHoliEnd(dto.getAppr_holi_end())
				.apprHoliUseCount(dto.getAppr_holi_use_count()).apprForm(apprFo).employee(emp).docuNo(documentNumber)
				.apprState(dto.getAppr_state() != null ? dto.getAppr_state() : "S")
				.isDeleted(dto.getIs_deleted() != null ? dto.getIs_deleted() : "N").build();
	...
	public Approval saveApproval(ApprovalDto approvalDto) {
		...
		Approval approval = Approval.builder().employee(employee).apprTitle(approvalDto.getAppr_title())
				.apprContent(approvalDto.getAppr_content()).apprWriterName(approvalDto.getAppr_writer_name())
				.apprHoliStart(approvalDto.getAppr_holi_start()).apprHoliEnd(approvalDto.getAppr_holi_end())
	...
	public ApprovalDto getDataInfo(ApprovalDto approvalDto) {
		...
		approvalDto.setAppr_writer_code(employee.getEmpCode());
		approvalDto.setAppr_writer_name(employee.getEmpName());
		ApprovalDto dto = ApprovalDto.builder().draft_day(approvalDto.getDraft_day())
				.appr_writer_name(approvalDto.getAppr_writer_name()).build();
	...
	public List<ApprovalDto> getAllApprovals(int size) {
		...
		List<Approval> apprList = approvalRepository.findByEmployeeAccountOrderByDraftDayDesc(currentUserId, pageable)
				.getContent();

		// Approval 엔티티를 ApprovalDto로 변환
		return apprList.stream().map(approval -> new ApprovalDto().toDto(approval)).collect(Collectors.toList());
	}
	...
	public List<ApprovalDto> getDraftListByEmpAccount(String empAccount) {
		return approvalRepository.findApprovalsByEmpAccount(empAccount).stream()
	...
	public TemporaryStorage updateAppr(TemporaryStorageDto dto) {
		...
		if (temp == null) {
			// Builder를 사용하여 객체 생성
			temp = TemporaryStorage.builder().apprTitle(dto.getAppr_title()).apprContent(dto.getAppr_content())
					.apprHoliStart(dto.getAppr_holi_start()).apprHoliEnd(dto.getAppr_holi_end())
					.apprHoliUseCount(dto.getAppr_holi_use_count()).build();
		} else {
			// 기존 객체 업데이트
	...
	public TemporaryStorageDto selectTemporaryStorageOne(Long tem_no) {
		...
		return TemporaryStorageDto.builder().tem_no(temp.getTemNo()).appr_title(temp.getApprTitle())
				.appr_content(temp.getApprContent()).appr_holi_start(temp.getApprHoliStart())
				.appr_holi_end(temp.getApprHoliEnd()).build();
	}
	...
	public List<TemporaryStorageDto> getAllTemporaryStorage(int size) {
		...
		List<TemporaryStorage> tempList = temporaryStorageRepository
				.findTop5ByEmployeeEmpAccountOrderByTemNoDesc(currentUserId, pageable).getContent();

		// Approval 엔티티를 ApprovalDto로 변환
		return tempList.stream().map(temporaryStorage -> new TemporaryStorageDto().toDto(temporaryStorage))
				.collect(Collectors.toList());
	}
	...
	public List<ApprovalDto> getPendingApprovalDtosForCurrentUser(int size) {
		...
		Page<Approval> pendingApprovals = approvalLineRepository
				.findPendingApprovalsForCurrentUser(currentUser.getEmpCode(), pageable);

		// 로그 출력 (선택 사항)
		System.out.println("결재 대기 문서 개수: " + pendingApprovals.getTotalElements());

		// Approval 엔티티를 ApprovalDto로 변환하여 반환
		return pendingApprovals.getContent().stream().map(approval -> new ApprovalDto().toDto(approval))
				.collect(Collectors.toList());
	}
	...
	public List<ApprovalLineDto> getApprovalLinesByApprNo(Long apprNo) {
		...
		return approvalLines.stream().map(approvalLine -> new ApprovalLineDto().toDto(approvalLine))
				.collect(Collectors.toList());
	}
	...
	private boolean isAllApprovalCompleted(Long apprNo) {
		...
		boolean allCompleted = approvalLines.stream().filter(line -> line.getApprRole() == 1) // 1은 결재자, 2는 참조자라고 가정
	...
	public ApprovalLine saveApprovalLine(ApprovalLineDto lineDto) {
		...
		System.out.println(
				"ApprovalLine Details: Order - " + lineDto.getAppr_order() + ", Role - " + lineDto.getAppr_role());

		if (lineDto.getAppr_role() == 1) {  // 결재자
		    if (lineDto.getAppr_order() == null) {
		        throw new IllegalArgumentException("결재자의 결재 순서(appr_order)가 설정되지 않았습니다.");
		    }
		    // appr_order가 1차 결재자일 경우
		    if (lineDto.getAppr_order() == 1) {
		        System.out.println("1차 결재자입니다.");
		    }
		}
		...
		ApprovalLine approvalLine = ApprovalLine.builder().apprOrder(lineDto.getAppr_order()) // 결재 순서 설정
	...
	public Approval getApprovalByStateAndNo(String apprState, Long apprNo) {
		return approvalRepository.findByApprStateAndApprNo(apprState, apprNo).orElseThrow(
				() -> new IllegalArgumentException("해당 결재 문서를 찾을 수 없습니다. 상태: " + apprState + ", 문서 번호: " + apprNo));
	}
	...
	public List<ApprovalDto> getReferenceListByEmpAccount(String empAccount) {
		...
		// Approval 엔티티를 ApprovalDto로 변환
		List<ApprovalDto> approvalDtos = approvals.stream().map(approval -> new ApprovalDto(approval)) // Approval에서
																										// ApprovalDto로
																										// 변환
				.collect(Collectors.toList());
	...
	public List<ApprovalDto> getStandByDraftListByEmpAccount(String empAccount) {
		...
		// Approval 엔티티를 ApprovalDto로 변환
		return approvals.stream().map(approval -> new ApprovalDto(approval)).collect(Collectors.toList());
	}
	...
	public List<ApprovalDto> getAllApprovalsByEmpCode(String empAccount) {
		...
		List<ApprovalLine> approvalLines = approvalLineRepository
				.findAllApprovalLinesByEmpCode(currentUser.getEmpCode());

		// ApprovalLine에서 Approval을 추출하여 ApprovalDto로 변환
		return approvalLines.stream().map(approvalLine -> new ApprovalDto(approvalLine.getApproval())) // ApprovalLine에서
																										// Approval을
																										// 추출하여 DTO로 변환
				.collect(Collectors.toList());
	}
